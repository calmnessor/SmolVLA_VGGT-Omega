...
The target ratio is approximately 0.1-0.2. Persistent cosine below -0.3 is evidence to reduce lambda. The initial candidate is `lambda_max=0.05`, subject to this diagnostic.
...
4. Verify only `SceneProjector`, auxiliary attention, and `DepthHead` receive depth gradients; VGGT and SmolVLA vision parameters do not.
5. Run the 100-500 step diagnostic before selecting the formal lambda.
