...
class WNMGeometryConditioner(nn.Module):
    ...
    def forward(self, history: Tensor) -> Tensor:
        ...
        with torch.no_grad(), torch.autocast(device_type=device_type, dtype=self.encoder_dtype, enabled=device_type == "cuda"):
        ...
        if tokens.ndim != 3 or tokens.shape[1] != expected_tokens or tokens.shape[2] != self.output_dim:
            raise ValueError(f"Expected geometry tokens [B,{expected_tokens},{self.output_dim}], got {tuple(tokens.shape)}")
